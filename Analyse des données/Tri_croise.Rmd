...
```{r all_stat_2023, eval=FALSE, include=FALSE}
library(glue)

# Stats centralisées dans tableau - édition 2023 
...
# Application des fonctions créées
    # Qs simples
library(purrr)
purrr::map(.x = c(13:16, 18, 46, 49, 55, 70, 111:113, 115, 139, 162, 174:178, 180, 181, 197),
...
table_recap_stat <- rbind(table_recap_stat, tot_plus3500) |> 
  ...
  #quelques nb totales de communes ne sont pas 30, on remplace (il n'y a pas eu d'incidence sur le pourcentage, bien calculé avec occurrences/30)
  mutate(`Effectifs par structure` = case_when(Structure == "Une commune" ~ 30,
                                               .default = `Effectifs par structure`))
    # Export
table_recap_stat |> rio::export("../Data/All_statistics_2023.csv")
```
...
```{r all_stat_2022, eval=FALSE, include=FALSE}
# On modifie la variable de type de structure pour correspondre à l'édition 2023
data_2022 <- readxl::read_xlsx("../Data/data_enquete_2022.xlsx", "Resultats Bruts") |> 
  mutate(Catégorie = str_replace_all(`Vousconnaitre1. Votre collectivité est :`, "Un EPCI \\(communauté de communes, communauté urbaine ou d’agglomération, métropole\\)", "Un EPCI"),
         Catégorie2 = case_when(is.na(`G1Q00004. Quel est le nombre d’habitants de votre commune ?`) ~ Catégorie,
                                .default = `G1Q00004. Quel est le nombre d’habitants de votre commune ?`),
         Catégorie2 = str_replace_all(Catégorie2, "Un EPCI \\(communauté de communes, communauté urbaine ou d’agglomération, métropole\\)", "Un EPCI"))

# On calcule les infos nécessaires
ref_nb_structures_2022 <- rbind(data_2022 |> summarise(n_total = n(), .by = Catégorie2),
                  data_2022 |> filter(Catégorie == "Une commune") |> summarise(n_total = n(), .by = Catégorie) |> rename(Catégorie2 = 1)) |>
    add_row(Catégorie2 = "Total", n_total = nrow(data_2022))
nb_commune_2022 <- (ref_nb_structures_2022 |> filter(Catégorie2 == "Une commune") |> select(n_total))$n_total

# Stats centralisées dans tableau - édition 2022
    # Fonctions qs simples
table_recap_simple22 <- function(n_col){
    name <- names(data_2022[,n_col])
    nb_communes <- (data_2022 |> 
        select(n_col, 226) |> 
        na.omit() |> 
        summarise(n_total = n(), .by = Catégorie) |> 
        filter(Catégorie == "Une commune"))$n_total
    ref_question <- data_2022 |> 
        select(n_col, 227) |> 
        na.omit() |> 
        summarise(n_total = n(), .by = Catégorie2)
    total <- sum(ref_question$n_total)
    ...
    cat1 <- data_2022 |> 
        ...
        pivot_longer(cols = -Catégorie2, names_to = "cat", values_to = "n") |> 
        mutate_all(replace_na, replace = 0) |>
        left_join(ref_nb_structures_2022, by = "Catégorie2") |> 
        mutate(percent = round(n / n_total *100, 0), .by = Catégorie2)
    cat2 <- data_2022 |> 
        select(n_col, Catégorie) |> 
        filter(Catégorie == "Une commune") |> 
        ...
        #mutate_all(replace_na, replace = 0) |>
        mutate(n_total = nb_communes,
               percent = round(n / 140 *100, 0), .by = Catégorie)
    lignes <- nrow(cat2)
    cat2[lignes+1, 1] <- "Une commune"
    cat2[lignes+1, 2] <- NA_character_
    cat2[lignes+1, 3] <- 0
    cat2[lignes+1, 4] <- 140 #140 commmunes en 2022
    cat2[lignes+1, 5] <- 0
    cat2 <- cat2 |> 
        filter(row_number() == 1, .by = 2)
    # All
    ...
    # Fonctions qs mutliples
table_recap_multiple22 <- function(min_col, max_col){
    name <- names(data_2022[,min_col]) |> str_extract("^.*\\[") |> str_replace_all("\\[", "")
    nb_communes <- (data_2022 |> 
        select(min_col:max_col, 226) |> 
        group_by(Catégorie) |> 
        filter(across(where(is.character), ~. != "N/A")) |> 
    ...
    cat2 <- data_2022 |> 
        ...
        mutate(cat = str_extract(cat, "(?<=\\[).*"),
               cat = str_replace_all(cat, "\\]", ""),
               #cat = replace_na(cat, "NA"),
               percent = round(n / 140 *100, 0), #140 communes en 2022
               n_total = 140) 
...
# Application des fonctions créées - édition 2022
    # Qs simples
library(purrr)
purrr::map(.x = c(20:23, 29, 48, 52, 54, 56, 61, 91, 93:95, 97, 98, 100, 101, 105, 107, 113:116, 134),
          .f = ~table_recap_simple22(.x))
table_qs_simples <- rbind(lapply(ls(pattern="^all_stat_"), function(x) get(x))) |> 
    bind_rows() |> 
    mutate(Réponses = replace_na(Réponses, "Sans réponse"))
    # Qs multiples
table_recap_multiple22(24,27)
table_recap_multiple22(33,44)
table_recap_multiple22(45,47)
table_recap_multiple22(62,66)
table_recap_multiple22(68,73)
table_recap_multiple22(75,78)
table_recap_multiple22(82,89)
table_recap_multiple22(108,112)
table_recap_multiple22(117,120)
table_recap_multiple22(122,130)
table_qs_multiple <- rbind(lapply(ls(pattern="^all_stat_multiples_"), function(x) get(x))) |>
    bind_rows() |> 
    mutate(Réponses = replace_na(Réponses, "Sans réponse"))
    # Tout ensemble
table_recap_stat <- rbind(table_qs_simples, table_qs_multiple)
    # Ajout Total hors communes de moins de 3500 hab
tot_plus3500 <- table_recap_stat |> 
  filter(Structure == "Total" | Structure == "Moins de 3 500 habitants") |> 
  pivot_wider(names_from = Structure, values_from = c(Occurrences, `Nombre de réponses`)) |> 
  mutate(Occurrences = sum(Occurrences_Total, -`Occurrences_Moins de 3 500 habitants`, na.rm = T), 
         `Nombre de réponses` = sum(`Nombre de réponses_Total`, -`Nombre de réponses_Moins de 3 500 habitants`, na.rm = T), 
         .by = c(Question, Réponses)) |> 
  distinct(Question, Réponses, Occurrences, `Nombre de réponses`) |> 
  mutate(`Effectifs par structure` = 297-81, #total - communes de moins de 3500 hab
         `Structure` = "Total hors communes < 3500 habitants",
         Pourcentage = round(Occurrences/216 *100, 0))
table_recap_stat <- rbind(table_recap_stat, tot_plus3500) |> 
  arrange(Question, Structure) |> 
  mutate(Edition = "2022") |> 
  #quelques nb totales de communes ne sont pas 140, on remplace (il n'y a pas eu d'incidence sur le pourcentage, bien calculé avec occurrences/30)
  mutate(`Effectifs par structure` = case_when(Structure == "Une commune" ~ 140,
                                               .default = `Effectifs par structure`))
    # Export
table_recap_stat |> rio::export("../Data/All_statistics_2022.csv")
```
