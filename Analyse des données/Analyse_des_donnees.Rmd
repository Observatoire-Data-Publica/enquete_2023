...
+ **Comparaison 2022 / 2022**

```{r}
# Stats pour graphique
table <- all_stat |> 
  filter(Question == "Commençons par la protection de la vie privée. Considérez-vous que votre structure est en conformité avec le RGPD ?" |
         Question == "Commençons par la protection de la vie privée. Considérez-vous que votre collectivité est en conformité avec le RGPD ?") |> 
...
table |> 
  ...
  ggplot(aes(x = Réponses, y = Pourcentage, fill = Edition, group = Edition)) +
    geom_bar(stat="identity", width=.6, size = 2, position = position_dodge2(preserve = "single", padding = 0)) +
    coord_flip() +
    ...
    scale_y_continuous(labels = scales::percent, limits = c(0, max(table$Pourcentage)+.1)) + # pourcentages
    ...
    geom_label(aes(y = pos_ecart, x = Réponses, label = ifelse(is.na(ecart), NA, paste0(ecart, "% par rapport \nà 2022"))), 
              color = "#333333", size = 4, check_overlap = T, fill = "white", label.size = NA, 
              hjust = 0)
