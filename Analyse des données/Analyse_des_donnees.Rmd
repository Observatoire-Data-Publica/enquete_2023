...
all_stat_2023 <- rbind(all_stat_2023, epci_met) |> 
...
all_stat <- rbind(all_stat_2022, all_stat_2023) |> 
...
saving_plot_comp <- function(graph, name) {
  ggsave(file = glue("../Graphiques/PNG/{name}.png"), plot = graph, width = 25, height = 15)
  ggsave(file = glue("../Graphiques/SVG/{name}.svg"), plot = graph, width = 25, height = 15)
}
```
...
dataviz_comp <- function(nom_col22, nom_col23){
  # Stats pour graphique
  table_all <- all_stat |> 
    filter(Question == nom_col22 | Question == nom_col23) |> 
  ...
  graph_total_plus3500 <- table_total_plus3500 |> 
    ggplot(aes(x = Réponses, y = Pourcentage, fill = Edition, group = Edition,
      ...
      labs(x = "", y = "Pourcentage des répondants", 
           subtitle = "Comparaison des éditions <span style='color: #33bbc9; font-size: 23pt;'>2022</span> et <span style='color: #323465; font-size: 23pt;'>2023</span>",
           title = "Collectivités hors communes de moins \nde 3500 habitants") +
      scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 40)) + 
      scale_y_continuous(labels = scales::percent, limits = c(0, max(table_total$Pourcentage)+.5*max(table_total$Pourcentage))) + # pourcentages
      scale_fill_manual(values = c("2022" = "#33bbc9", "2023" = "#323465")) +
      theme_classic() +
  ...
  graph_coll <- table_all |> 
    ...
    mutate(Structure = factor(Structure, levels = c("Autre","Une région","Un département","Un EPCI","Une commune"))) |> 
      ggplot(aes(x = Structure, y = Pourcentage, fill = Réponses,
        ...
        geom_bar(position = "fill", color = "white", stat="identity", aes(alpha = Edition != "2023")) +
        labs(x = "", y = "Pourcentage de réponses", title = "Par type de collectivité") +
        scale_y_continuous(labels = scales::percent, limits = c(0,1)) +
        scale_fill_manual(values = c("grey", "#bf0001", "#e1b44d", "#33bbc9", "#323465", "#616165", "#0a0a14")) +
  ...
  #right_part
    # Save graphe
  assign("graph", all, envir = .GlobalEnv)
}
dataviz_comp("Commençons par la protection de la vie privée. Considérez-vous que votre structure est en conformité avec le RGPD ?",
             "Commençons par la protection de la vie privée. Considérez-vous que votre collectivité est en conformité avec le RGPD ?")
saving_plot_comp(graph, "Comparaison 2022-2023 : Commençons par la protection de la vie privée. Considérez-vous que votre structure est en conformité avec le RGPD ?")
```
...
```{r fig.height=14, fig.width=20}
dataviz_comp("Avez-vous nommé un(e) Délégué(e) à la Protection des Données (DPO) ?",
             "Avez-vous nommé un(e) Délégué(e) à la Protection des Données (DPO) ?")
saving_plot_comp(graph, "Comparaison 2022-2023 : Avez-vous nommé un(e) Délégué(e) à la Protection des Données (DPO) ?")
```
...
```{r fig.height=14, fig.width=20}
dataviz_comp("Afin de respecter le RGPD, avez-vous modifié des processus de gestion des données au sein de la collectivité ?",
             "Afin de respecter le RGPD, avez-vous modifié des processus de gestion des données au sein de l'organisation ?")
saving_plot_comp(graph, "Comparaison 2022-2023 : Afin de respecter le RGPD, avez-vous modifié des processus de gestion des données au sein de l'organisation ?")
```
...
```{r fig.height=14, fig.width=20}
dataviz_comp("De manière générale, avez-vous mis en place des clauses juridiques sur la gestion des données dans les contrats de délégation et dans les marchés publics ?",
             "De manière générale, avez-vous mis en place des clauses juridiques sur la gestion des données dans les contrats de délégation et dans les marchés publics ?")
saving_plot_comp(graph, "Comparaison 2022-2023 : De manière générale, avez-vous mis en place des clauses juridiques sur la gestion des données dans les contrats de délégation et dans les marchés publics ?")
```
...
```{r fig.height=14, fig.width=20}
dataviz_comp("Quels sont le ou les domaines concernés par ces clauses ?",
             "Quels sont le ou les domaines concernés par ces clauses ?")
saving_plot_comp(graph, "Comparaison 2022-2023 : Quels sont le ou les domaines concernés par ces clauses ?")
```
...
```{r fig.height=14, fig.width=20}
dataviz_comp("La question de la souveraineté numérique préoccupe de plus en plus d'acteurs publics : choix des éditeurs de logiciels, hébergement des données en Europe ou en France, maîtrise des données par la collectivité... Votre collectivité a-t-elle défini une politique en matière de souveraineté publique sur les données ?",
             "La question de la souveraineté numérique préoccupe de plus en plus d'acteurs publics : choix des éditeurs de logiciels, hébergement des données en Europe ou en France, maîtrise des données par la collectivité... Votre organisation a-t-elle défini une politique en matière de souveraineté publique sur les données ?")
saving_plot_comp(graph, "Comparaison 2022-2023 : Votre organisation a-t-elle défini une politique en matière de souveraineté publique sur les données ?")
```
...
dat22 <- data_2022 |> 
    ...
    mutate(cat = str_extract(cat, "(?<=\\[).*"),
           ...
           type = "2022") |> 
    select(-n) |> mutate(Réponses = "Oui", labelpos = NA_real_)
dat23 <- all_stat_2023 |> 
    filter(grepl("Pour chacun de ces domaines, pouvez-vous préciser si votre structure a engagé ces deux dernières années un ou plusieurs projets ou expérimentations en matière d'utilisation des données, ou prévoit d’en engager au cours des douze prochains mois ?", Question) == TRUE & 
             Structure != "Communes de moins de 3 500 hab." & Structure != "Total" & Structure != "Une commune" & Structure != "Total pondéré" & 
...
graph <- comp_editions |> 
    ggplot(aes(y=Pourcentage, x=cat, fill = Réponses, group = Réponses)) + 
            ...
            geom_text(aes(x = cat, y = Pourcentage/2, label = ifelse(type == "2022", paste0(Pourcentage, "%"), NA)), 
                       size = 6, hjust = 0, color = "white") +
            geom_text(aes(x = cat, y = labelpos, label = paste0(Pourcentage, "%")), 
                       size = 6, hjust = 0, color = "white") + 
            scale_x_discrete(labels = function(x) stringr::str_wrap(x, width = 30)) +
...
```{r eval=FALSE, fig.height=14, fig.width=20, include=FALSE}
dataviz_comp("La question de la souveraineté numérique préoccupe de plus en plus d'acteurs publics : choix des éditeurs de logiciels, hébergement des données en Europe ou en France, maîtrise des données par la collectivité... Votre collectivité a-t-elle défini une politique en matière de souveraineté publique sur les données ?",
             "Pour chacun de ces domaines, pouvez-vous préciser si votre structure a engagé ces deux dernières années un ou plusieurs projets ou expérimentations en matière d'utilisation des données, ou prévoit d’en engager au cours des douze prochains mois ? [Aménagement du territoire]")
saving_plot_comp(graph, "Comparaison 2022-2023 : La question de la souveraineté numérique préoccupe de plus en plus d'acteurs publics : choix des éditeurs de logiciels, hébergement des données en Europe ou en France, maîtrise des données par la collectivité... Votre organisation a-t-elle défini une politique en matière de souveraineté publique sur les données ?")
```
